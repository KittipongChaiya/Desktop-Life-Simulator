...
| 13  | **Catch-up over-credits by one harvest** — `PLAN.md` §8 criterion 14           | Found by the property test during phase-08.0c on seed `1435051507`, path `5:0:0:1:1:1:5:4:5:5:4:5:5:6:5:9:9` — one worker, a seed bin, two crops, 10 wheat and 2 turnip seeds. Reproduces with every 08.0c change stashed, so it is latent rather than introduced | Catch-up credits more than running the ticks does (12 vs 11). NOT data loss — the player gains, and nothing on disk is damaged                      | Fix `catch-up.ts` with the counterexample above as the regression test. **Then** item 14                                                                                                                                                              |
| 14  | **The two `assertNeverOver` properties carry no pinned seed**                  | The other five properties in `catch-up.test.ts` pin 424242, 7, 7, 11 and 99; these two sample fresh farms on every run, against `TESTING.md` §6.2 — _"always pass an explicit seed"_                                                                              | A release-gate criterion that passes by sampling is not a gate: it was green for months with item 13 latent, and is green or red between runs       | Pin both seeds once item 13 is fixed, and treat a new counterexample as a finding to fix rather than noise between runs                                                                                                                               |
