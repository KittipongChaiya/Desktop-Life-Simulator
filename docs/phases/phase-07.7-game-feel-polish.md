...
| 13  | **Catch-up over-credits** — `PLAN.md` §8 criterion 14. **PARTIALLY FIXED (09c)** | TWO distinct defects wore one symptom. (a) `cropStats.planted`, seed `1435051507`: the model credited a final replant the window had no time for, then back-dated it to `end` with a `Math.min` — a clamp accepting an impossible timestamp instead of refusing the action. **Fixed**, with a pinned regression. (b) `cropStats.harvested`, seed `-1749120238`: still open | (a) closed. (b) credits 14 harvests where the real simulation manages 13, on a farm with ONE worker and TWO crop tiles                              | Investigate (b) as tile-independent cadence: the model gives every tile its own full `growthTicks + CYCLE_HANDLING_TICKS` cycle simultaneously, as though one worker served each, while the only shared limit is an energy budget that never binds. A single worker interleaving two tiles cannot achieve that |
| 14  | **The two `assertNeverOver` properties carry no pinned seed**                    | The other five in `catch-up.test.ts` pin 424242, 7, 7, 11 and 99; these two sample fresh farms every run, against `TESTING.md` §6.2                                                                                                                                                                                                                                        | A release-gate criterion that passes by sampling is not a gate — and it is currently the only thing still finding defect 13(b)                      | Pin both once 13(b) is fixed. Pinning now would either freeze a passing seed (hiding the defect) or freeze a failing one (a permanently red suite). Until then the suite is intermittently red, and that is the honest state                                                                                   |
