...
ADR-001 accepts PixiJS _conditionally_, on the mandatory render-on-demand constraint. Implementing the dirty gate before the first sprite is drawn means it is never retrofitted around existing animation code — which is how it would decay.
...
- Crops, growth, or any farming _(phase-03)_
- Workers, entities, or movement _(phase-04)_
- Inventory or buildings _(phase-05, 06)_
- Particles, weather, or lighting — **layers 4–5 stay empty** _(v0.2)_
- Vertical panning — the overlay is 220 px tall; the world fits vertically
- Tile editing by the player _(phase-03 adds tilling)_
- Minimap _(v0.4)_
...
## Completion Status (phase-02 close-out)

| #   | Criterion                                          | Status                                                                                 |
| --- | -------------------------------------------------- | -------------------------------------------------------------------------------------- |
| 1   | 64x64 grid renders with correct tile art           | **PASS** — E2E, backend reported                                                       |
| 2   | Owned 8x8 plot visually distinct                   | **PASS** — unowned tiles tinted in the cached chunk texture, so it costs no draw calls |
| 3   | Camera pans smoothly, clamped                      | **PASS** — 5 unit tests                                                                |
| 4   | No shimmering while panning                        | **PASS** — snapping asserted against whole _device_ pixels at 1x and 2x DPI            |
| 5   | **Zero rAF callbacks over 10s, static world**      | **PASS** — 6 unit tests + live E2E (`FPS 0.0`, `Dirty no · 0 anim`)                    |
| 6   | One tile change marks the scene dirty exactly once | **PASS** — unit                                                                        |
| 7   | One tile change re-renders exactly one chunk       | **PASS** — unit + E2E (steady-state redraws 0)                                         |
| 8   | Static farm under 30 draw calls                    | **BLOCKED** — see below                                                                |
| 9   | Texture memory within budget                       | **BLOCKED**                                                                            |
| 10  | Collapse destroys the Pixi app                     | **PASS** — teardown implemented and exercised on every toggle                          |
| 11  | Expand re-initializes repeatedly                   | **PASS** — mount/unmount driven by collapse state                                      |
| 12  | Expand <300ms, collapse <100ms                     | **BLOCKED**                                                                            |
| 13  | Expanded idle CPU within budget                    | **BLOCKED** — needs baseline hardware                                                  |
| 14  | Expanded RSS within budget                         | **BLOCKED**                                                                            |
| 15  | Frame time under 16ms while panning                | **BLOCKED**                                                                            |
| 16  | Forced fallback renders layers 0-3 and 6           | **PASS** — the fallback is what this environment runs; ADR-001 §Implementation note    |
| 17  | No texture referenced by string literal            | **PASS** — lint                                                                        |
| 18  | 20 collapse/expand cycles leak nothing             | **BLOCKED**                                                                            |

### Blocked criteria

8, 9, 12, 13, 14, 15, and 18 all require a **GPU-backed session on baseline
hardware**. This environment exposes no WebGL or WebGPU adapter, so PixiJS runs
its Canvas fallback, where draw calls and texture memory are not meaningful
measures and timings measure the wrong thing.

They are **written and skipped with a stated reason**, not omitted
(`tests/e2e/render-budget.spec.ts`). Each un-skips automatically once
`hasGpuBackend()` is true — no edit required. A missing test is
indistinguishable from an untested requirement; a skipped one is a standing
instruction.

**These must be run on a GPU machine before v0.1 ships** (`PLAN.md` §8 gates
performance on measured numbers, and every budget in `PERFORMANCE.md` is still
marked provisional).

### Deferred to phase-03, with reason

- **Vertical panning** — out of scope by design; the overlay is 220px and the
  world fits vertically.
- **Layers 4-5 content** — created empty on purpose (v0.2 particles/lighting).
- **Tilled/path tile kinds** — registered by phase-03 with the systems that give
  them meaning; registering them now would be content without behaviour.
