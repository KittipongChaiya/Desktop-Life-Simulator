...
**Should phase-05 be adjusted?** Yes, in framing, not scope. Phase-05 should be implemented **as "Resources & Inventory" on top of this ADR** — the general `Container`/`ItemStack`/`transfer` primitive first, with player inventory and worker carry as its first two owners — rather than a one-off inventory that phases 06 and v0.4 would later have to generalize. The deliverables in `docs/phases/phase-05-inventory.md` do not change; the internal structure they build does. The phase document should reference this ADR as its governing model.
