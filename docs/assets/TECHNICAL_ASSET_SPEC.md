...
## Numeric glyphs (phase-07.7c)

Eleven glyphs — `0`–`9` and `plus` — in the `ui-world` atlas, generated by
`scripts/generate-glyph-art.mjs`.

|         |                                    |
| ------- | ---------------------------------- |
| Canvas  | 7×9                                |
| Core    | 5×7, hand-set per glyph            |
| Colour  | Parchment, **tinted at draw time** |
| Outline | full 1 px, per `ICON_GUIDE.md`     |

**Why a font rather than `Text`.** Pixi rasterises a texture per distinct
string, so `+12` and `+13` would be two GPU allocations on the most frequent
effect in the game. Eleven textures composed into pooled per-digit sprites
allocate nothing after startup.

**Why 5×7.** A 3×5 core was drawn first and rejected on sight: with the canon
1 px outline the border is as thick as the strokes, so `+` collapsed into a blob
and the digits read only barely. The outline is canon, so the core grew.

**Why no `×`.** It was drawn and cut. At this size its diagonals sit one pixel
apart and the outline closes them into a solid block. Item counts and coins both
read as `+n`, so nothing needs it — and a speculative glyph is not worth an
illegible one.

Tinting at draw time is what keeps the set at eleven: a coin number and an item
number differ only by tint, never by texture.
