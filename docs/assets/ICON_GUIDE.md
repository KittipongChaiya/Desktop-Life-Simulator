...
| **UI chrome**       | v0.1+  | Interface buttons and panel toggles (inventory, settings): one quiet glyph on the parchment chrome (`UI_STYLE_GUIDE.md §2–4`); part of the UI surface, never competing with world or item icons. Files `icon_ui_<name>.png` (`NAMING_CONVENTION.md §4.4`). _Added in phase-05.6c: the categories above had no home for chrome glyphs._ |
