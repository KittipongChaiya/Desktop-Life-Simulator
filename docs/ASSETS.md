...
### 7.3 Character art — and the placeholders it replaced

`assets/src/entities{tps}/` holds **production** character art, generated by
`scripts/generate-character-art.mjs`: six entities — three worker rigs, the
player, and two villagers — each with four directions, an idle frame and a
four-frame walk. Workers and the player also get a harvest pose; villagers do
not, because nothing ever asks a villager to farm. The `.anim.json` sidecars
are emitted by the same script, which is what fixed the defect where
hand-authored sidecars meant a new rig rendered nothing at all.

Three worker rigs exist so that a farm stops being staffed by one person
printed several times; which rig a worker gets is a hash of its id, derived and
never stored.

#### The placeholders are gone, and the reason is worth keeping

This section described 16×16 placeholders from phase-04c until the v0.5 RC —
three phases after `generate-character-art.mjs` replaced them. The promise the
placeholders were built on was the right one and it was kept: _production art
replaces them with no code change, because the renderer resolves sprites
through the generated manifest and reads frames from the sidecar, never from a
filename or a coordinate._

But the two placeholder generators —
`generate-placeholder-worker-art.mjs` (phase-04c) and
`generate-placeholder-item-building-art.mjs` (phase-05d) — **were still on
disk, and still wrote the same filenames as the production generators.**
Running either reverted live art: the worker rig to grey blobs and one
animation short, or the storage shed and four item icons to their stand-ins.
They were deleted at phase 52.

`tests/art-regeneration.test.ts` now runs every art generator and fails if a
single committed byte moves, which is ADR-006 §2's guarantee stated as a test
rather than as an intention. That is the thing to keep: a superseded generator
is not harmless just because nobody runs it on purpose.
